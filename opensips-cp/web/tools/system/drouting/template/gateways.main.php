...
<?php csrfguard_generate();
 ...
 $search_gwid=isset($_SESSION['gateways_search_gwid']) ? $_SESSION['gateways_search_gwid'] : "";
 if ($search_gwid!="") {
 ...
 $search_type=isset($_SESSION['gateways_search_type']) ? $_SESSION['gateways_search_type'] : "";
 if ($search_type!="") {
	 $sql_search.=" and type=?";
	 array_push( $sql_vals, $search_type);
 }

 $search_address=isset($_SESSION['gateways_search_address']) ? $_SESSION['gateways_search_address'] : "";
 if ($search_address!="") {
	 $sql_search.=" and address like ? ";
	 array_push( $sql_vals, "%".$search_address."%");
 } 
 
 $search_pri_prefix=isset($_SESSION['gateways_search_pri_prefix']) ? $_SESSION['gateways_search_pri_prefix'] : "";
 if ($search_pri_prefix!="") {
	 $sql_search.=" and pri_prefix=?";
	 array_push( $sql_vals, $search_pri_prefix);
 }

 $search_probe_mode=isset($_SESSION['gateways_search_probe_mode']) ? $_SESSION['gateways_search_probe_mode'] : "";
 if ($search_probe_mode!="") {
	$sql_search.=" and probe_mode=?";
	array_push( $sql_vals, $search_probe_mode);
 }

 $search_description=isset($_SESSION['gateways_search_description']) ? $_SESSION['gateways_search_description'] : "";
 if ($search_description!="") { 
	$sql_search.=" and description like ?";
	array_push( $sql_vals, "%".$search_description."%");
 }

 $search_attrs=isset($_SESSION['gateways_search_attrs']) ? $_SESSION['gateways_search_attrs'] : "";
 if ($search_attrs!="") {
...
<?php csrfguard_generate(); ?>
  ...
  <input onclick="openCsvUploadDialog()" name="csv_upload" class="formButton" value="Upload CSV" type="button"/>
</form>
<?php } ?>
...
if (!empty($mi_connectors) && isset($mi_connectors[0])) {
	$message=mi_command( "dr_gw_status", $params, $mi_connectors[0], $errors);

	if (!is_null($message) && isset($message['Gateways']) && is_array($message['Gateways'])) {
...
if ($memory_status != "0") {
   $gw_status = isset($gw_statuses[$resultset[$i]['gwid']]) ? $gw_statuses[$resultset[$i]['gwid']] : "n/a";
